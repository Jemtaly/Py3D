...
class Camera:
    def __init__(self, centre = numpy.zeros(3), matrix = numpy.eye(3)):
        self.centre = centre
        self.matrix = matrix
    def transform(self, absolute):
        return self.matrix.dot(absolute - self.centre)
    def move(self, mv):
...
class Tk3D:
    ...
    def turn(self, event):
        ...
        self.camera.rota(numpy.array([rtx, rty, 0.0]))
        self.turn_evrec = event
        self.refresh()
    def tilt(self, event):
        rtz = math.atan2(self.tilt_evrec.y - self.cy, self.tilt_evrec.x - self.cx) - math.atan2(event.y - self.cy, event.x - self.cx)
        self.camera.rota(numpy.array([0.0, 0.0, rtz]))
        self.tilt_evrec = event
        self.refresh()
    def move(self, event):
        mvx, mvy = (self.move_evrec.x - event.x) / self.size, (event.y - self.move_evrec.y) / self.size
        self.camera.move(numpy.array([mvx, mvy, 0.0]))
        self.move_evrec = event
        self.refresh()
    def wheel(self, event):
        mvz = (0 - event.delta or event.num * 240 - 1080) / self.size
        self.camera.move(numpy.array([0.0, 0.0, mvz]))
        self.refresh()
