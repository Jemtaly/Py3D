...
def main():
    ...
    def plot(mode):
        xs = np.linspace(xmin_slider.value(), xmax_slider.value(), xnum_slider.value() + 1, endpoint=True)
        ys = np.linspace(ymin_slider.value(), ymax_slider.value(), ynum_slider.value() + 1, endpoint=True)
        try:
